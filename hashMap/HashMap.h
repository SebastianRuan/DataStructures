...
class HashMap{
    ...
    std::pair<unsigned long int, std::string>* arr;
    int capacity;
    int count;
    const long double phi = (sqrt(5.0)-1.0)/2.0;

    unsigned long int flatten(std::string value);
    int hash1(unsigned long int key);
    int hash2(unsigned long int key);
    int cuckooHash(std::vector<int> indices, unsigned long int key, const std::string& value);
    void clear_table();
